...
//    if ($find != '') {
    if ($z_testid_pk == '') {
        ...
        if ($leavel == 'brands') {
            $testsize_query = "select count(*) as cnt from  questions where questionType!='users' AND parent_qa_id=0 and query!='' AND channel='$channel'";
        } else {
            $testsize_query = "select count(*) as cnt from  questions where questionType='users' AND parent_qa_id=0 and query!='' AND channel='$channel' ";
        }
        $testsize_query_array = select("$testsize_query");
        $qatestsize = $testsize_query_array[0]['cnt'];
        if ($qatestsize < $maintestsize) {
            $maintestsize = $qatestsize;
        }
...
//            }
//        }
        else if ($extra_id != '' && $qa_id == 13) {//facebook if comment is 1
//            $assoc['extra_id'] = '';
//            $outcomment = trim($result[0][1]);
            if ($expectedoutput == '') {
                $expectedoutput = 0;
            }
            if ($find == $expectedoutput) {
//                $assoc['extra_id'] = $expectedoutput;
//                $extra_id = $expectedoutput;
//                    echo pass($ans_id);
                pass($assoc['z_testid_pk'], $contactId, $qa_id, $handler, $extra_id, $find, $expectedoutput);
...
//            }
            if (!is_numeric($expectedoutput)) {
                ...
                $expectedoutput = remove_extra_spaces($expectedoutput);
...
//                mysql_query($insert_query);
                ...
                $tolerans = $question_row[0]['tolerance'] * 100;
                $range = ceil(($expectedoutput * $tolerans) / 100);

                $new1 = $expectedoutput + $range;
                $new2 = $expectedoutput - $range;
...
function pass($z_testid_pk, $contactId, $qa_id, $handler, $extra_id, $find, $expectedoutput) {
    ...
    if ($insert_update == 'insert') {
        ...
        $query = "UPDATE  answer SET z_contactid_fk='$contactId',date='" . getdatetime() . "',postID='$extra_id',passed='1',input='" . mysql_real_escape_string($find) . "',expected='" . mysql_real_escape_string($expectedoutput) . "' WHERE z_testid_fk='$z_testid_pk' AND qa_id='" . $qa_id . "' AND handle='$handler' AND postID='$extra_id'";
        mysql_query($query);
    }
...
function get_tablewise_samplesize($type) {
//    global $assoc;/
    $tablewise_samplesize = "SELECT * from tablewise_samplesize  WHERE questionType='$type'  order BY  tablesampleSize_id";
    $tablewise_samplesize_array = select($tablewise_samplesize);
    $sampleSize = trim($tablewise_samplesize_array[0]['sampleSize']);
    $passrate = trim($tablewise_samplesize_array[0]['passrate']);
    $mainrate = ceil($passrate * $sampleSize);
    return $mainrate;
}
...
//          $assoc['getresult_pass'] = $outresult;
        if ($passed == 1) {
            $assoc['outputresult'] = 'pass';
//            update_logtable("pass");
        } else {
            $assoc['outputresult'] = 'fail';
//            update_logtable("fail");
        }
...
function update_logtable($currentqaresult) {
    global $channel, $z_testid_pk, $assoc, $alreadypassed, $qa_id;
    ...
    $score_updated_status = "SELECT score_updated from answer where z_testid_fk='$z_testid_pk' AND qa_id='$qa_id' LIMIT 1 ";
    $score_updated_status_val = select($score_updated_status);
    $score_updated = $score_updated_status_val[0][0];
...
//    $score = $preScore+1;
//    $preScore++;
    $assoc['$currentqaresult'] = $currentqaresult;
    $assoc['$alreadypassed'] = $alreadypassed;
    if ($currentqaresult == 'pass' && $alreadypassed == 'fail') {
        $query = "UPDATE  answer SET score_updated='1' where z_testid_fk='$z_testid_pk' AND qa_id='$qa_id'";
//        echo $query;
//        exit();
        $assoc['updarescorequery'] = $query;
        mysql_query($query);
        $score = ++$preScore;
        $newStatus = '';
        $assoc['$score'] = $score;
    ...
    } else if ($currentqaresult == 'pass' && $alreadypassed == 'pass') {
        $newScore = $preScore;
        $newStatus = $logArr[0]['status'];
    } else if ($currentqaresult == 'fail' && $alreadypassed == 'fail') {
        $newScore = $preScore;
        $newStatus = $logArr[0]['status'];
    } else if ($currentqaresult == 'fail' && $alreadypassed == 'pass') {
        $score = --$preScore;
        $newStatus = '';
        $assoc['$score'] = $score;
        $assoc['$passrate'] = $passrate;
...
//        $newStatus = $logArr[0]['status'];
        ...
        $query = "UPDATE  answer SET score_updated='0' where z_testid_fk='$z_testid_pk' AND qa_id='$qa_id'";
        mysql_query($query);
    }
...
function update_score() {
    global $channel, $z_testid_pk, $assoc, $alreadypassed, $handler;
    $sql = "SELECT DISTINCT (a.qa_id), q.questionType FROM answer a JOIN questions AS q ON q.qa_id = a.qa_id WHERE z_testid_fk =$z_testid_pk";
    $arr = select($sql);
    $j = 0;
    for ($i = 0; $i < count($arr); $i++) {
        $qa_id = $arr[$i]['qa_id'];
        $questionType = trim($arr[$i]['questionType']);
        if ($questionType == 'brands' || $questionType == 'historical') {
            $find_pre_qa = "SELECT passed from answer where z_testid_fk='$z_testid_pk' AND qa_id='" . $qa_id . "' AND handle='$handler' limit 1";
            $outresult = select($find_pre_qa);
            $passed = $outresult[0][0];
            if ($passed == 1) {
            ...
            $psrate = get_tablewise_samplesize($questionType);
            $find_pre_qa = "SELECT count(*) as cnt from answer where z_testid_fk='$z_testid_pk' AND qa_id='" . $qa_id . "' AND handle='$handler' AND passed='1'";

            $outresult = select($find_pre_qa);
            $passed = $outresult[0][0];
            $assoc['getresult_pass'] = $psrate;
            if ($passed >= $psrate) {
                $j++;
            }
    ...
    $q = "UPDATE `logtable` SET score=$j ,status='$newStatus' WHERE `z_testid_pk`='$z_testid_pk'";
    $assoc['mainupdate'] = $q;
    mysql_query($q);
}
...
function remove_extra_spaces($find) {
    $find = trim($find);
    $find = str_replace("\n", ' ', $find);
    $find = str_replace("\r", ' ', $find);
    $find = str_replace("\r\n", ' ', $find);
    $text = preg_replace('/\s+/', ' ',$find);
//    $text = preg_replace("/[^\p{L}\p{N}]/u", ' ', $find);
    return $text;
}
