...
//    
    $res = mysql_query("(
SELECT max(l.checked_date) as checked_date,l.channel,l.z_testid_pk,l.score,l.status,c.username,s.testsize,s.passrate,l.level FROM `logtable` as l  join contact as c on l.z_contactid_fk=c.z_contactid_pk 
...
//    $query = "SELECT DISTINCT(handle) from $channels[$brand].brands";
    $query = "SELECT l.z_testid_pk,l.checked_date,l.channel,l.score,l.level,l.status,c.username,s.testsize,s.passrate 
FROM `logtable` as l  
...
//    $random_handle = $randome_handle_array[0]['handle'];
    ...
    $randuserquery = "SELECT friendly,handle,userID FROM $channels[$channel_name]" . ".users WHERE friendly != '' AND handle != '' ORDER BY rand() LIMIT 1 ";
...
//            $query = "SELECT q.qa_id,q.channel,q.question,q.query,q.url,q.tolerance,q.passed,q.sampleSize,q.parent_qa_id,q.parent_passrate,q.questionType,t.sampleSize as st,t.passrate as tt FROM `questions` as q JOIN tablewise_samplesize as t ON t.questionType=q.questionType WHERE q.channel='$channel_name' and q.parent_qa_id=0 and q.query!='' order by  rand() limit $testsize";
        ...
        if ($channel_name == 'instagram') {
            $fetchrandom_postID = "select postID,content from $channels[$channel_name].posts  where handle= '$searchhandle' AND likes>0 AND comments>0 ORDER BY rand() LIMIT 1";
            ...
            $fetchrandom_postID = "select postID,content,link from $channels[$channel_name].posts  where handle= '$searchhandle' AND engagements>10 ORDER BY rand() LIMIT 1";
            $fetchrandom_postIDarray = select($fetchrandom_postID);
            $assoc['postIdarray'] = $fetchrandom_postIDarray;
        }
        if ($channel_name != 'instagram') {
            if ($channel_name == 'twitter') {
                $fetchrandomuser_postID = "select up.postID,up.content,up.link,u.handle from $channels[$channel_name].userposts  as up join $channels[$channel_name].users as u ON up.userID=u.userID where up.handle= '$searchhandle' AND up.content!=''  ORDER BY rand() LIMIT 1";
                $fetchrandomuser_postIDarray = select($fetchrandomuser_postID);
                $assoc['userpostIdarray'] = $fetchrandomuser_postIDarray;
            } else {
                $fetchrandomuser_postID = "select postID,content,link from $channels[$channel_name].userposts  where handle= '$searchhandle' AND content!=''  ORDER BY rand() LIMIT 1";
                $fetchrandomuser_postIDarray = select($fetchrandomuser_postID);
                $assoc['userpostIdarray'] = $fetchrandomuser_postIDarray;
            }
...
//            if()
        ...
        if ($channel_name == 'twitter') {
            $fetchrandom_engagementID = "select e.engagementID,u.handle from $channels[$channel_name].engagements as e join $channels[$channel_name].users as u ON e.userID=u.userID where e.handle= '$searchhandle' AND e.content!='' ORDER BY rand() LIMIT 1";
            $fetchrandom_engagementIDarray = select($fetchrandom_engagementID);
            $assoc['engagementIdarray'] = $fetchrandom_engagementIDarray;
        }


        $query = "SELECT q.qa_id,q.channel,q.question,q.query,q.url,q.tolerance,q.passed,q.sampleSize,q.field,q.parent_qa_id,q.parent_passrate,q.questionType,t.sampleSize as st,t.passrate as tt FROM `questions` as q JOIN tablewise_samplesize as t ON t.questionType=q.questionType WHERE q.channel='$channel_name' and q.parent_qa_id=0 and q.query!='' AND q.questionType!='users' order by  rand() limit $testsize";
...
//        $assoc['question'][] = $value;
    ...
    $query = "SELECT a.`qa_id` , a.`z_testid_fk` , a.`expected` , a.`input` , a.`passed` ,a.handle,q.`channel`, q.`question` , q.`sampleSize` , q.`parent_qa_id` , q.`parent_passrate` 
            FROM answer a
...
//    if ($leavel == 'brands') {
    ...
    if ($leavel == 'brands') {
        ...
        /* fetch total test size from setting ::::::  start */
        $settingQue = "SELECT testsize
	FROM setting
	WHERE channel='" . $channel_name . "'";

        $total_testsize = select($settingQue);
        $no_total_testsize = $total_testsize[0]['testsize'];

        if ($no_total_testsize > $no_of_ansed_que) {
            $testsize = ($no_total_testsize - $no_of_ansed_que);
        }
        /* fetch total test size from setting ::::::  end */
        ...
        foreach ($parent_answered_que as $value) {
            ...
            if ($value['sampleSize'] > 1) {
                $q = "SELECT * FROM `questions` WHERE `channel` = '$channel_name' and `parent_qa_id`=" . $value['qa_id'] . " and `query`!=''";
                $subqueArr = select($q);
                foreach ($subqueArr as $subvalue) {
                    $allQue[] = $subvalue;
                }
...
//        $assoc['question'][] = $value;
        ...
        $ansed_ids = implode(',', $ansed_ids);

        /*
         * fetch answer detail of answered question
         */
        $qu = "SELECT * FROM `answer` WHERE `z_testid_fk` =$z_testid_fk AND `handle` = '" . $brand_name . "' and qa_id IN ($ansed_ids) order by qa_id";
        $ans = select($qu);
        $answers = array();
        ...
        for ($i = 0; $i < count($parent_answered_que); $i++) {
            $class = '';
            if ($ans[$i]['passed'] == -1) {
                $class = 'fail_paginate';
            ...
            $next = $i + 1;
            $pagenation.="<li><a onclick='getPageData($next)' class='$class' id='page_$next' href='javascript:void(0);' class='pages'>$next</a></li>";
        }



        $assoc['handle'] = $brand_name;
        $assoc['channel'] = $channel_name;
        $query = "SELECT testsize,passrate from setting WHERE channel='$channel_name'";
        $result = select($query);
//    $testsize = $result[0]['testsize'];
        $assoc['testsize'] = $no_total_testsize;
        $assoc['passrate'] = $result[0]['passrate'];

        $quer = "SELECT * FROM `questions` WHERE `channel` = '$channel_name' and `parent_qa_id`=0 and `query`!='' and qa_id NOT IN (" . $ansed_ids . ") order by  rand() limit $testsize";
        $queArr = select($quer);
        $assoc['quesize'] = count($queArr) + $no_of_ansed_que - 1;
        $mainQueSize = count($queArr) + $no_of_ansed_que - 1;

        if (is_array($queArr) && isset($queArr)) {
            foreach ($queArr as $value) {
                $allQue[] = $value;
                if ($value['sampleSize'] > 1) {
                    $q = "SELECT * FROM `questions` WHERE `channel` = '$channel_name' and `parent_qa_id`=" . $value['qa_id'] . " and `query`!=''";
...
//        $assoc['question'][] = $value;
        ...
        for ($i = $no_of_ansed_que; $i < count($queArr) + $no_of_ansed_que - 1; $i++) {
            $next = $i + 1;
            $pagenation.="<li><a onclick='getPageData($next)' id='page_$next' href='javascript:void(0);' class='pages'>$next</a></li>";
        }
        $pagenation.="<li><a onclick='' id='nextPage' href='javascript:void(0);'> &gt; </a></li>
            <li><a onclick='getPageData($mainQueSize)' href='javascript:void(0);'> &gt;&gt; </a></li></ul>";
        $assoc['allquesize'] = count($allQue);
        ...
        $settingQue = "SELECT sampleSize,passrate
	FROM tablewise_samplesize
	WHERE questionType='users'";

        $total_testsize = select($settingQue);
        $no_total_testsize = $total_testsize[0]['sampleSize'];
        $lt = $no_total_testsize - 1;
        foreach ($parent_answered_que as $value) {
            $allQue[] = $value;
            $ansed_ids[] = $value['qa_id'];
            if ($value['sampleSize'] > 1) {
                $userqaid = $value['qa_id'];
                $q = "SELECT * FROM `questions` WHERE `channel` = '$channel_name' and `parent_qa_id`=" . $value['qa_id'] . " and `query`!='' ORDER BY qa_id  LIMIT $lt";
                $subqueArr = select($q);
                foreach ($subqueArr as $subvalue) {
                    $allQue[] = $subvalue;
                }
                $assoc['innerquefind'][] = $q;
            }
////        $assoc['question'][] = $value;
        }
...
//            }
        $assoc['quesize'] = 1;
        $next = 1;
        $pagenation.="<li><a onclick='getPageData($next)' class='$class users' id='page_$next' href='javascript:void(0);' class='pages'>$next</a></li>";
//        }
        $pagenation.="<li><a onclick='' id='nextPage' href='javascript:void(0);'> &gt; </a></li>
  <li><a onclick='getPageData(1)' href='javascript:void(0);'> &gt;&gt; </a></li></ul>";
        $assoc['allquesize'] = count($allQue);
        $assoc['question'] = $allQue;
        $assoc['pagination'] = $pagenation;
        $assoc['leavel'] = 'users';
...
//        $result = select($query);
//        $assoc['testsize'] = $no_total_testsize;
//        $assoc['passrate'] = ceil($result[0]['passrate'] * $no_total_testsize);
//    } else {
//        $query = "SELECT sampleSize,passrate
//	FROM tablewise_samplesize
//	WHERE questionType='users'";
//        $result = select($query);
//        $assoc['testsize'] = 1;
//        $assoc['passrate'] = ceil($result[0]['passrate'] * $result[0]['sampleSize']);
//    }
//
////    $testsize = $result[0]['testsize'];
//    if ($leavel == 'brands') {
...
//    } else {
//
//        $quer = "SELECT * FROM `questions` WHERE qa_id='$userqaid'";
//        $queArr = select($quer);
//        $assoc['quesize'] = count($queArr) + $no_of_ansed_que - 1;
//        $mainQueSize = count($queArr) + $no_of_ansed_que - 1;
//
...
//                }
                ...
                if (($post[0][0] > 0) && ($userpost[0][0] > 0) && ($engage[0][0] > 0)) {
                    $newHandles[] = $handle;
                }
